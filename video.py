...
class Video:
    ...
    def _move_origin_to_top_left(self, y0, dy):
        return self.get_height() - y0, -dy
    
    def show_with_vectors(self, vectors, title):
        _wait_user()
        for index, frame in enumerate(self.get_frames()):
            for vector in vectors:
                ...
                if _is_valid_vector(x0s, y0s, dxs, dys, index):
                    y0_converted, dy_converted = self._move_origin_to_top_left(y0s[index], dys[index])
...
def _draw_vector(frame, x0, y0, dx, dy, tag, color):
        ...
        cv.circle(frame, (x0, y0), 5, color, -1)
