...
class Data:
    ...
    def _add_velocity(self):
        for item in self.objects:
            self.df[f"vx_{item}"] = self.df[f"rx_{item}"].diff()
            self.df[f"vy_{item}"] = self.df[f"ry_{item}"].diff()
        self.df = ft.apply_filter(self.df, cf.velocity_columns(self.df.columns))

    def _add_acceleration(self):
        for item in self.objects:
            self.df[f"ax_{item}"] = self.df[f"vx_{item}"].diff()
            self.df[f"ay_{item}"] = self.df[f"vy_{item}"].diff()
    ...
    def _add_polar_wrist(self):
        ...
        magnitude_vector_elbow_to_wrist = np.sqrt(
            x_vector_elbow_to_wrist ** 2 + y_vector_elbow_to_wrist ** 2)
        ...
        magnitude_vector_elbow_to_shoulder = np.sqrt(
            x_vector_elbow_to_shoulder ** 2 + y_vector_elbow_to_shoulder ** 2)

        elbow_to_wrist_scalar_product_elbow_to_shoulder = (
            x_vector_elbow_to_wrist * x_vector_elbow_to_shoulder +
            y_vector_elbow_to_wrist * y_vector_elbow_to_shoulder)
        ...
        sin_angle_upperarm_vertical = (
            distance_shoulder_vertical / magnitude_vector_elbow_to_shoulder)
        ...
        self.df['angle_forearm_vertical'] = (
            self.df['theta_wrist'] + self.df['angle_upperarm_vertical'])
        self.df['angle_upperarm_horizontal'] = (
            self.df['theta_wrist'] + self.df['angle_forearm_horizontal'])
    ...
    def _add_other_positions(self):
        ...
        self.df['rx_forearm'] = self.df['rx_elbow'] + (
            (self.df['rx_wrist'] - self.df['rx_elbow']) / 2)
        self.df['ry_forearm'] = self.df['ry_elbow'] + (
            (self.df['ry_wrist'] - self.df['ry_elbow']) / 2)
        self.objects.append("forearm")
